...
'''
    ...
    '''
'''   
    #acp diff modification, we do not need any modification or padding. lets try use a simple cross entropy loss -hadi
    #recon_loss = F.cross_entropy(logits,sequences,reduction='mean')
    #loss = nn.MSELoss()

    #tiara edit
    #recon_loss = F.mse_loss(logits,sequences)

    return F.cross_entropy(logits.view(-1, logits.size(-1)), sequences.view(-1), ignore_index=pad, reduction='mean')
...
def recon_dec(sequences, logits):
    ...
    # and sequences shape is [batch_size, sequence_length, vocab_size]
    
    # Convert sequences to class indices if it's one-hot encoded
    if sequences.dim() == 3:
    ...
    # Reshape sequences to [batch_size * sequence_length]
    sequences = sequences.reshape(-1)
    
    # Cross-entropy loss
    return F.cross_entropy(logits, sequences, ignore_index=pad, reduction='mean')
...
def wae_mmd_gaussianprior(z, method='full_kernel', sigma=7):
    ...
    if method == 'full_kernel':
        return mmd_full_kernel(z, z_prior, sigma=sigma, kernel='gaussian')
    else:
        return mmd_rf(z, z_prior, sigma=sigma, kernel='gaussian', rf_dim=500, rf_resample=False)

def mmd_full_kernel(z1, z2, sigma, kernel):
    K11 = compute_mmd_kernel(z1, z1, sigma, kernel)
    K22 = compute_mmd_kernel(z2, z2, sigma, kernel)
    K12 = compute_mmd_kernel(z1, z2, sigma, kernel)
    N = z1.size(0)
    assert N == z2.size(0), 'expected matching sizes z1 z2'
    H = K11 + K22 - K12 * 2
    H = H - torch.diag(H)
    return 1. / (N * (N - 1)) * H.sum()

def mmd_rf(z1, z2, sigma, kernel, rf_dim, rf_resample=False):
    mu1 = compute_mmd_mean_rf(z1, sigma, kernel, rf_dim, rf_resample)
    mu2 = compute_mmd_mean_rf(z2, sigma, kernel, rf_dim, rf_resample)
    return ((mu1 - mu2) ** 2).sum()

rf = {}

def compute_mmd_mean_rf(z, sigma, kernel, rf_dim, rf_resample):
    global rf
    if kernel == 'gaussian':
        if not kernel in rf or rf_resample:
            rf_w = torch.randn((z.shape[1], rf_dim), device=z.device)
        ...
        raise ValueError('todo implement rf for kernel ' + kernel)
    return z_rf.mean(0, keepdim=False)

def compute_gaussian_rf(z, rf_w, rf_b, sigma, rf_dim):
    z_emb = (z @ rf_w) / sigma + rf_b
    return torch.cos(z_emb) * (2. / rf_dim) ** 0.5
...
def compute_mmd_kernel(x, y, sigma, kernel):
    ...
    xmy = ((x_i - y_j) ** 2).sum(2)
    if kernel == "gaussian":
        return torch.exp(- xmy / sigma ** 2)
    elif kernel == "laplace":
        return torch.exp(- torch.sqrt(xmy + (sigma ** 2)))
    elif kernel == "energy":
        return torch.pow(xmy + (sigma ** 2), -.25)
    else:
