...
class Unet(Module):
    def __init__(
        ...
        #v2
        self.mid_attn = FullAttention_ca(in_channels = mid_dim, n_heads = attn_heads[-1], d_head = attn_dim_head[-1],context_dim=context_dim)
...
class Trainer:
    def __init__(
        ...
        *,
        ...
        valid_dl = DataLoader(valid_dataset, batch_size=valid_batch_size, shuffle=False, pin_memory=True, num_workers=cpu_count())
    ...
    def validate(self):
        ...
        with torch.no_grad():
            for idx, (data, context, _) in enumerate(self.valid_dl):
    ...
    def sample(self):
            raise Exception("Trainer sample method deprecated")
            self.model.eval()
            try:
                with torch.no_grad():
                    ...
                    for batch_size in batches:
                        context = next(self.train_dl)[1]
                        context = context.to(self.device)

                        if context.size(0) != batch_size:
                            context = context[:batch_size]  # Ensure context matches the batch size
